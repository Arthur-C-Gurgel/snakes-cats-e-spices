...
describe("Prato", () => {
  ...
  it("deve criar uma Prato com os dados corretos", () => {
    const t = new Prato(1, "Estudar Jest", false, "01/06/2026");
  ...
  it("deve rejeitar titulo vazio", () => {
    const erros = Prato.validar({ titulo: "" });
...
it("deve converter de JSON para instancia (fromJSON)", () => {
    ...
    const t = Prato.fromJSON(json);
  
    expect(t).toBeInstanceOf(Prato);
    expect(t.id).toBe(5);
    expect(t.titulo).toBe("Teste");
  }); 
  
  it("deve converter para JSON (toJSON)", () => {
    const t = new Prato(1, "Jest", false, "01/06/2026", null);
    const json = t.toJSON();
...
it("deve lançar erro ao setar titulo vazio", () => {
  const t = new Prato(1, "Teste", false);
